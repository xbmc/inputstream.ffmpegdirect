...
#include "platform/ThreadImpl.cpp"
...
void CThread::Create(bool bAutoDelete)
{
  if (m_thread != nullptr)
  {
    ...
    { // otherwise we have a problem.
      Log(LOGLEVEL_ERROR, "%s - fatal error creating thread %s - old thread id not null", __FUNCTION__, m_ThreadName.c_str());
      exit(1);
    }
  }
...

  {
    ...
    m_thread = new std::thread([](CThread* pThread, std::promise<bool> promise)
    {
      try
      {
        ...
        if (pThread == nullptr)
        {
          Log(LOGLEVEL_ERROR,"%s, sanity failed. thread is NULL.",__FUNCTION__);
        ...
        Log(LOGLEVEL_DEBUG,"Thread %s start, auto delete: %s", name.c_str(), (autodelete ? "true" : "false"));
        ...
        if (autodelete)
        {
          Log(LOGLEVEL_DEBUG,"Thread %s %s terminating (autodelete)", name.c_str(), id.c_str());
          delete pThread;
          pThread = NULL;
        }
        else
        {
          Log(LOGLEVEL_DEBUG,"Thread %s %s terminating", name.c_str(), id.c_str());
        }
      }
      catch (const std::exception& e)
      {
        Log(LOGLEVEL_DEBUG,"Thread Terminating with Exception: %s", e.what());
      }
      catch (...)
      {
        Log(LOGLEVEL_DEBUG,"Thread Terminating with Exception");
      }
